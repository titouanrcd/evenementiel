...
<head>
    ...
    <!-- Google Maps API -->
    <script async defer src="https://maps.googleapis.com/maps/api/js?key=VOTRE_CLE_API"></script>
    
    <style>
        /* Mini popup itinéraire */
        .event-location {
            position: relative;
        }
        .location-popup {
                                ...
                                <article class="event-card <?php echo ($index === 0) ? 'featured' : ''; ?>">
                                    ...
                                    <div class="event-info">
                                        ...
                                        <p class="event-location">
                                            <span class="location-link" onclick="toggleLocationPopup(this, '<?php echo htmlspecialchars(addslashes($event['lieu'])); ?>')">
                                            ...
                                            <div class="location-popup">
                                                <div class="popup-title">Itinéraire</div>
                                                <div class="popup-modes">
                                                    <div class="popup-mode active" data-mode="driving" title="Voiture">
                                                    ...
                                                    <div class="popup-mode" data-mode="transit" title="Transports">
                                                        <svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2c-4.42 0-8 .5-8 4v9.5C4 17.43 5.57 19 7.5 19L6 20.5v.5h12v-.5L16.5 19c1.93 0 3.5-1.57 3.5-3.5V6c0-3.5-3.58-4-8-4zM7.5 17c-.83 0-1.5-.67-1.5-1.5S6.67 14 7.5 14s1.5.67 1.5 1.5S8.33 17 7.5 17zm3.5-6H6V6h5v5zm2 0V6h5v5h-5zm3.5 6c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5z"/></svg>
                                                    </div>
                                                    <div class="popup-mode" data-mode="walking" title="À pied">
                                                    ...
                                                    <div class="popup-mode" data-mode="bicycling" title="Vélo">
                                                        <svg viewBox="0 0 24 24" fill="currentColor"><path d="M15.5 5.5c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zM5 12c-2.8 0-5 2.2-5 5s2.2 5 5 5 5-2.2 5-5-2.2-5-5-5zm0 8.5c-1.9 0-3.5-1.6-3.5-3.5s1.6-3.5 3.5-3.5 3.5 1.6 3.5 3.5-1.6 3.5-3.5 3.5zm5.8-10l2.4-2.4.8.8c1.3 1.3 3 2.1 5.1 2.1V9c-1.5 0-2.7-.6-3.6-1.5l-1.9-1.9c-.5-.4-1-.6-1.6-.6s-1.1.2-1.4.6L7.8 8.4c-.4.4-.6.9-.6 1.4 0 .6.2 1.1.6 1.4L11 14v5h2v-6.2l-2.2-2.3zM19 12c-2.8 0-5 2.2-5 5s2.2 5 5 5 5-2.2 5-5-2.2-5-5-5zm0 8.5c-1.9 0-3.5-1.6-3.5-3.5s1.6-3.5 3.5-3.5 3.5 1.6 3.5 3.5-1.6 3.5-3.5 3.5z"/></svg>
                                                    </div>
                                                </div>
                                                <div class="popup-info">
                                                    <span class="popup-distance"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path></svg> <span class="distance-value">--</span></span>
                                                    <span class="popup-duration"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg> <span class="duration-value">--</span></span>
                                                </div>
                                                <a href="#" class="popup-btn" target="_blank">
    ...
    <script>
        ...
        function initDirections() {
            if (typeof google !== 'undefined' && google.maps) {
                ...
                console.log('⚠️ Google Maps API non disponible - le bouton redirigera directement');
            }
        }
        
        // Toggle le popup
        function toggleLocationPopup(element, destination) {
            console.log('📍 Clic sur:', destination);
            
            // Le popup est le frère suivant du span cliqué
            const popup = element.nextElementSibling;
            
            if (!popup || !popup.classList.contains('location-popup')) {
                console.error('Popup non trouvé');
                // Fallback: ouvrir Google Maps directement
                window.open('https://www.google.com/maps/dir/?api=1&destination=' + encodeURIComponent(destination), '_blank');
                return;
            }
            
            // Fermer le popup actif s'il y en a un autre
            ...
            activePopup = isOpening ? popup : null;
            
            if (isOpening) {
                initPopup(popup, destination);
            }
        }
        
        // Initialiser un popup
        function initPopup(popup, destination) {
            const modes = popup.querySelectorAll('.popup-mode');
            ...
            function updateLink() {
                ...
                btn.href = 'https://www.google.com/maps/dir/?api=1&destination=' + encodedDest + '&travelmode=' + currentMode;
            }
            
            // Calculer la distance/durée
            function calculateRoute() {
                const distanceEl = popup.querySelector('.distance-value');
                const durationEl = popup.querySelector('.duration-value');
                
                if (!directionsService || !userLocation) {
                ...
                directionsService.route({
                    ...
                    travelMode: modeMap[currentMode]
                }, function(result, status) {
                    if (status === 'OK') {
                        const route = result.routes[0].legs[0];
                        distanceEl.textContent = route.distance.text;
                        durationEl.textContent = route.duration.text;
                    } else {
                        distanceEl.textContent = '--';
                        durationEl.textContent = '--';
                    }
            ...
            // Empêcher le clic sur le bouton de fermer le popup
        ...
        document.addEventListener('click', function(e) {
            if (activePopup && !e.target.closest('.location-link') && !e.target.closest('.location-popup')) {
                activePopup.classList.remove('active');
                activePopup = null;
            }
        });
        ...
        // Aussi essayer après un délai si l'API charge en async
        setTimeout(initDirections, 1000);
    </script>
